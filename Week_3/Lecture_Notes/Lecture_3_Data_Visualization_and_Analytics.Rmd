...
```
Great! It does not look like we have any missing values in the age variable. Before subsetting our data frame on age, let's also discuss the distribution of the systolic blood pressure variable. Judging by the histogram and violin plot, the distribution of the heart rate variable is fairly normal. According to the cumulative distribution function, about 90% of the values are between 100 and 150. As we did with age, let's take a look and see if there are any NA values in our systolic blood pressure variable.
...
ggplot(data_df_sub, aes(x = RIDAGEYR, y = BPXOSY1)) +
  ...
  theme_minimal()
```
The plot we created above indicates that for every year of increase in age, systolic blood pressure increases by 0.4 beats per minutes. The positive relationship is highlighted by the slight up slope of the regression line. 
...
We search for the corresponding demographic and examination data frames for a single year in their respective vectors of data frames and merge those data frames by the participant ID, "SQEN". This combined data frame is then added to the vector or `merged_dfs` under the name of the examination data frame for that year. 
...
### Heatmaps
 ...
 I'm not saying anything leading, but this part **might** be helpful for your Final Project Check In 2 and Final Project. I have generated some pseudo RNA-seq count data along with corresponding meta data. I want to get an idea of the relationships between my genes by Disease Status and Sex. The best visualization to do this is a heat map. A heat map  is a graphical representation of data where individual values contained in a matrix are represented as colors. In RNA-seq data, heat maps are often used to display the expression levels of genes (rows) across multiple samples (columns). The color intensity reflects the magnitude of the values allowing for easier interpretation of patterns. 
 
Let's create a heat map of just 10 of our genes and add annotation bars for Sex and Disease Status in order to gain an understanding of how those variables contribute to gene expression. 
...
counts <- read.csv("/Users/f002yt8/Documents/GitHub/HDS-Foundations_of_Data_Science/Week_3/Data/heatmap_counts.csv", row.names = 1)

metadata <- read.csv("/Users/f002yt8/Documents/GitHub/HDS-Foundations_of_Data_Science/Week_3/Data/heatmap_metadata.csv", row.names = 1)

## I'm just selecting 10 genes here
...
* Advanced Data Wrangling and Visualization: We looped through two different types of files to implement built in functions as well as our custom functions. We also briefly discuss how to generate heat maps!
